...
const issueTreeProvider_1 = require("./providers/issueTreeProvider");
const testTreeProvider_1 = require("./providers/testTreeProvider");
const usageTreeProvider_1 = require("./providers/usageTreeProvider");
const diagnostics_1 = require("./diagnostics");
let client;
let diagnosticManager;
let issueTreeProvider;
let testTreeProvider;
let usageTreeProvider;
let statusBarItem;
/**
 * 拡張機能アクティベート時の処理
 */
async function activate(context) {
    ...
    // クライアント初期化
    client = new client_1.DevBuddyClient();
    diagnosticManager = new diagnostics_1.DiagnosticManager();
    // ツリービュープロバイダー初期化
    issueTreeProvider = new issueTreeProvider_1.IssueTreeProvider();
    testTreeProvider = new testTreeProvider_1.TestTreeProvider();
    usageTreeProvider = new usageTreeProvider_1.UsageTreeProvider(client);
    // ツリービュー登録
    context.subscriptions.push(vscode.window.registerTreeDataProvider('devbuddy.issues', issueTreeProvider), vscode.window.registerTreeDataProvider('devbuddy.tests', testTreeProvider), vscode.window.registerTreeDataProvider('devbuddy.usage', usageTreeProvider));
    // ステータスバーアイテム作成
    ...
    // コマンド登録
    context.subscriptions.push(vscode.commands.registerCommand('devbuddy.reviewFile', reviewCurrentFile), vscode.commands.registerCommand('devbuddy.reviewSelection', reviewSelection), vscode.commands.registerCommand('devbuddy.generateTests', generateTests), vscode.commands.registerCommand('devbuddy.suggestFix', suggestFix), vscode.commands.registerCommand('devbuddy.setApiKey', setApiKey), vscode.commands.registerCommand('devbuddy.showUsage', showUsage));
...
async function reviewDocument(document) {
    ...
    if (!supportedLanguages.includes(document.languageId)) {
        vscode.window.showWarningMessage(`DevBuddyAI: ${document.languageId}は現在サポートされていません`);
        return;
    }
    // ステータスバー更新
    ...
    try {
        await vscode.window.withProgress({
            ...
            if (issueCount === 0) {
                vscode.window.showInformationMessage('DevBuddyAI: 問題は検出されませんでした');
            }
            else {
                vscode.window.showInformationMessage(`DevBuddyAI: ${issueCount}件の問題を検出しました`);
            }
        });
    }
    catch (error) {
    ...
    try {
        await vscode.window.withProgress({
            ...
            const result = await client.generateTests(code, {
                language: editor.document.languageId,
                framework: config.get('testFramework') || 'pytest',
            ...
            vscode.window.showInformationMessage(`DevBuddyAI: ${result.tests.length}件のテストを生成しました`);
        });
    }
    catch (error) {
...
async function suggestFix() {
    ...
    try {
        ...
        await vscode.window.withProgress({
            ...
            progress.report({ increment: 100 });
            // 修正提案を表示
            const action = await vscode.window.showInformationMessage(`DevBuddyAI: 修正提案（信頼度: ${Math.round(result.confidence * 100)}%）`, '適用', '詳細表示', 'キャンセル');
            if (action === '適用') {
                // 修正を適用
...
function showResultPanel(title, content) {
    const panel = vscode.window.createWebviewPanel('devbuddyResult', title, vscode.ViewColumn.Beside, { enableScripts: true });
